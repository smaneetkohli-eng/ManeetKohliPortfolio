...
| 1 | `#bio` | — | `makeEdgeWaves` | Maneet's statement, four thin paragraphs between two white edge waves |
...
- `.bio__statement`: four `.bio__line` paragraphs (Maneet's statement), one size, Inter 300 at `clamp(17px, 1.65vw, 27px)`, a 58ch column centred under the dock. Each paragraph is its own shimmer via `background-clip: text` on the `<p>` (not the `data-text` copy, so the `.bio__accent` serif italics wrap with the rest), delays staggered 0.35s per paragraph so the sweep flows down the page. No kicker any more.
