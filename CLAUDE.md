...
- **Mark previews** (`.card__preview--resume`, `.card__preview--regal`): no words. One illustrated mark rests in the centre of the preview (SVG viewBox 220×100, 78% wide). On `.card:hover` it travels right (CSS `transform` on the `<g>`, SVG user units, 1s) while a line draws in behind it (`stroke-dashoffset` 1 → 0 on a `pathLength="1"` path, same 1s); leaving retracts both. Rest and hover transforms live in CSS, not the `transform` attribute.
  - Resume Agent: white sheet, black ink. A fountain pen (`.card__pen`, nib at the path start, white separation strokes via `paint-order: stroke`) writes the ink line (`.card__trail--ink`, plus `.card__trail-dot`).
  - Regal: craft paper (`#efe5cf` + the same noise as the page, multiplied), maroon ink, a dashed inner border. A folded paper plane (`.card__plane`, geometry centred on its own origin, two paper tones) flies right off a dotted flight line revealed through a mask (`.card__trail-mask`, id `rg-trail`).
...
`index.html` links `css/site.css?v=29` and `js/site.js?v=12`. **Whenever you touch either file, bump its number in `index.html`** or the browser serves stale code. (The legacy `styles.css?v=` / `main.js?v=` numbers on the archived subpages no longer matter.)
