...
- **Title**: `.projects__title` ("Projects and Experiences") is centred under the dock on every step, in the bio kicker's voice (Inter 500, 14px, 0.26em tracking, uppercase, shimmer). White on the dark steps, maroon ink on Regal and near-black on Resume Agent via `[data-project="2"]` / `[data-project="3"]`. `.page--projects` has 5vh of top padding so the slab sits a touch lower under it.
- **Tags are descriptions, not specs**: a few plain words on what the project is (e.g. "Tailored resumes", "Your own AI agent", "For students"). Never tech stacks. Four per card, always one row: `.card__tags` sets `font-size: clamp(9px, 0.72vw, 12px)` and the tags' padding, gap and radius are in em, so the whole row scales with the card. At the narrowest card (440px) the four Bani AI tags use ~98% of the row; anything longer needs a shorter word, not a smaller font.
- **Resume Agent card**: `.card__preview--resume` is a white sheet with RESUME above and AGENT below in the hero's Archivo (size `--word-size`, 15% of `--card-w`) and `.card__icon` across the middle: a sheet, a pen laid over it and two sparks, black fills with white separation strokes (`paint-order: stroke`) so it reads over the type.
...
- Placeholders: Authentic Intelligence and Regal have no link and empty tinted previews. Contact in the dock goes nowhere yet.
...
`index.html` links `css/site.css?v=24` and `js/site.js?v=12`. **Whenever you touch either file, bump its number in `index.html`** or the browser serves stale code. (The legacy `styles.css?v=` / `main.js?v=` numbers on the archived subpages no longer matter.)
