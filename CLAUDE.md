...
| Fonts | Archivo 900 (wdth 100–125, display) + Inter 400/500 (body) + Instrument Serif italic (bio accent) via Google Fonts. Card previews only: Special Elite (typewriter, RESUME / AGENT) and Caveat 600 (handwriting, "Internship"), loaded in a second Google Fonts link subset to those letters (`text=`). |
...
- **Word-and-mark previews** (`.card__preview--resume`, `.card__preview--regal`): a big word above and below (`--word-size`, a fraction of `--card-w`) with an illustrated `.card__icon` across the middle. **Trails are hidden at rest** (`stroke-dashoffset: 1` on a `pathLength="1"` path) and transition to 0 on `.card:hover`, so they draw in on enter and retract on leave. Hover also eases the words apart 4px.
  - Resume Agent: white sheet, black ink, RESUME / AGENT in Special Elite (typewriter). A fountain pen (`.card__pen`, white separation strokes via `paint-order: stroke`) sits on the end of the line it draws on hover (`.card__trail--ink`, plus `.card__trail-dot` at the start); the pen nudges as the line completes.
  - Regal: craft paper (`#efe5cf` + the same noise as the page, multiplied), maroon ink, a dashed inner border, REGAL in Archivo and "Internship" in Caveat handwriting. A folded paper plane (`.card__plane`, two paper tones) sits on a dotted flight line revealed through a mask (`.card__trail-mask`, id `rg-trail`); hover draws the line and lifts the plane.
...
`index.html` links `css/site.css?v=27` and `js/site.js?v=12`. **Whenever you touch either file, bump its number in `index.html`** or the browser serves stale code. (The legacy `styles.css?v=` / `main.js?v=` numbers on the archived subpages no longer matter.)
